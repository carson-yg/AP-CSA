...
public class Picture extends SimplePicture 
{
	...
	public void negate() {
         for (Pixel p1: this.getPixels()){
             int red = (int)(255 - p1.getColor().getRed());
             int green = (int)(255 - p1.getColor().getGreen());
             int blue = (int)(255 - p1.getColor().getBlue());
             p1.setColor(new Color(red, green, blue));
         }
	}
	...
	*/
	public void grayScale() {
         for (Pixel p1: this.getPixels()){
             int red = (int)(p1.getColor().getRed() * 0.3);
             int green = (int)(p1.getColor().getGreen() * 0.59);
             int blue = (int)(p1.getColor().getBlue() *0.11);
             int grey = red + green + blue;
             p1.setColor(new Color(grey, grey, grey));
         }
	}
    ...
    for (int col = 1; col < 329; col++)
		{
			// loop from 13 to just before the mirror point
			for (int row = 1; row < mirrorPoint; row++){
    ...
    int diff = 0;
      for (int x = 0; x < height; x++){
        for (int y = 0; y < width-1; y++){
          ...
          rightPixel = pixels[x][y+1];
          diff = (int) leftPixel.colorDistance(rightPixel.getColor());
            if (diff > edgeDist){
